...
exports.register = function (server, options, next) {
  server.route({
    ...
    handler: function (request, reply) {
      ...
      let sqlQuery = {
        type: 'select',
        query: 'SELECT universeId, name, description, chatId FROM aquest_schema.universe WHERE universeId=\'' + request.params.universeId + '\''
      };
      
      return new Promise(function(resolve, reject) {
        // Appel de l'action
        dbCaller.queryDb(sqlQuery).then(function(result) {
